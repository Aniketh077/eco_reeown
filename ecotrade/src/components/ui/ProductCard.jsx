  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 group relative flex flex-col h-full">
      ...
      <div className="relative flex-shrink-0">
        <Link to={`/product/${product._id}`}>
          <div className="aspect-square bg-gray-50 p-2 sm:p-3 overflow-hidden" style={{ maxHeight: '200px' }}>
            <img
              src={product.image}
              alt={product.name}
      ...
      <div className="p-2.5 sm:p-3 flex flex-col h-full">
        ...
        <Link to={`/product/${product._id}`} className="flex-1 flex flex-col min-h-0 mb-1.5">
          <h3 className="font-normal mb-1 line-clamp-2 text-[13px] sm:text-sm hover:text-green-700 transition-colors break-words overflow-hidden leading-snug">
        ...
        <div className="flex items-center mb-1.5 flex-shrink-0">
          <div className="flex items-center flex-wrap">
            {renderStars(product.rating || 0)}
            <span className="ml-1 text-[11px] text-gray-600 whitespace-nowrap">
              ({product.reviewCount || 0})
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-2 flex-shrink-0">
          {product.discountPrice ? (
            <div className="flex items-baseline gap-1.5 flex-wrap">
              <span className="text-sm sm:text-base font-medium text-green-700 whitespace-nowrap">
                ₹{product.discountPrice.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
              </span>
        ...
        <div className="flex items-center gap-1.5 mt-auto flex-shrink-0">
          <button
            disabled={product.stock === 0}
            onClick={handleAddToCart}
            className={`flex-1 py-1.5 text-[11px] sm:text-xs rounded transition-all duration-200 flex items-center justify-center ${
              product.stock === 0
          ...
          <button
            onClick={handleToggleWishlist}
            disabled={isWishlistLoading}
            className="bg-white border border-gray-300 rounded p-1.5 hover:border-red-400 transition-all duration-200 flex-shrink-0"
            aria-label={isInWishlist ? 'Remove from wishlist' : 'Add to wishlist'}
          >
            <Heart
